    ...
    <x-layout-admin>
        ...
        <div class="px-3 sm:px-4 lg:px-6 max-w-7xl mx-auto py-4 sm:py-6">
            ...
            <nav class="mb-4 sm:mb-6">
                <ol class="flex items-center space-x-2 text-sm">
                    <li><a href="{{ route('admin.questionnaire.index') }}" class="text-blue-600 hover:underline">Kuesioner</a>
                    </li>
            ...
            <!-- Alert Messages -->
            @if (session('success'))
                <div class="bg-green-100 border border-green-400 text-green-700 px-3 sm:px-4 py-3 rounded mb-4"
                    id="success-alert">
            ...
            @if ($errors->has('duplicate_quick'))
                <div class="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded flex items-center">
                    <i class="fas fa-exclamation-circle mr-2"></i>
                    {{ $errors->first('duplicate_quick') }}
                </div>
            @endif
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md border border-gray-200">
                <div class="px-4 sm:px-6 py-4 sm:py-6 border-b border-gray-200">
                    ...
                    <p class="text-xs sm:text-sm text-gray-600 mt-1">Buat periode kuesioner baru dengan target alumni yang
                        spesifik</p>
                </div>

                <form id="questionnaire-form" action="{{ route('admin.questionnaire.store') }}" method="POST" class="px-4 sm:px-6 py-4 sm:py-6">
                    @csrf
                    ...
                    <div class="mb-4 sm:mb-6">
                        ...
                        <div class="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                            <!-- All Alumni -->
                            <label
                                class="flex items-start p-3 sm:p-4 bg-blue-50 rounded-lg border-2 cursor-pointer transition-all hover:bg-blue-100"
                                id="target-all">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-blue-600 mt-1">Kuesioner dapat diakses oleh semua alumni
                                        terdaftar</p>
                                </div>
                            </label>
                            ...
                            <!-- Years Ago -->
                            <label
                                class="flex items-start p-3 sm:p-4 bg-green-50 rounded-lg border-2 cursor-pointer transition-all hover:bg-green-100"
                                id="target-years-ago">
                                <input type="radio" name="target_type" value="years_ago"
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-green-600 mt-1">Target alumni berdasarkan berapa tahun lalu
                                        mereka lulus (relatif dengan tahun sekarang: {{ now()->year }})</p>
                                </div>
                            </label>
                        ...
                        <!-- Years Ago Options -->
                        <div id="years-ago-section"
                            class="hidden border border-green-200 rounded-lg p-3 sm:p-4 bg-green-50">
                            <div
                                class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 space-y-2 sm:space-y-0">
                                <label class="text-sm font-medium text-green-800">
                                    Pilih Alumni yang Lulus Berapa Tahun Lalu
                                </label>
                                <div class="flex space-x-2 text-xs">
                                    <button type="button" id="select-all-years-ago"
                                        class="text-green-600 hover:text-green-800 font-medium">
                                        Pilih Semua
                                    </button>
                                    <span class="text-green-400">|</span>
                                    <button type="button" id="deselect-all-years-ago"
                                        class="text-green-600 hover:text-green-800 font-medium">
                                        Hapus Semua
                                    </button>
                                </div>
                            ...
                            <div class="mb-4">
                                <div class="relative">
                                    <input type="text" id="search-years-ago" placeholder="Cari tahun atau periode..."
                                    ...
                                    <button type="button" id="clear-search-years-ago"
                            ...
                            @if ($yearsAgoOptions->isNotEmpty())
                                <div id="years-ago-container"
                                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 max-h-60 sm:max-h-80 overflow-y-auto">
                                    @foreach ($yearsAgoOptions as $option)
                                        <label
                                            class="years-ago-item flex items-center justify-between p-3 bg-white rounded border cursor-pointer hover:bg-green-50 transition-colors"
                                            data-search-text="{{ $option['years_ago'] }} tahun lalu {{ $option['year'] }}">
                                            <div class="flex items-center flex-1 min-w-0">
                                                <input type="checkbox" name="years_ago_list[]"
                                                ...
                                                <div class="ml-2 min-w-0">
                                                    <span
                                                        class="text-sm font-medium text-gray-700 block truncate">{{ $option['years_ago'] }}
                                                        tahun lalu</span>
                                                    <p class="text-xs text-gray-500">({{ $option['year'] }})</p>
                                                </div>
                                            </div>
                                            <span
                                                class="text-xs text-green-600 bg-green-100 px-2 py-1 rounded flex-shrink-0 ml-2">
                                                {{ $option['count'] }}
                                            </span>
                                        </label>
                                    @endforeach
                        ...
                        <div id="specific-years-section"
                            class="hidden border border-purple-200 rounded-lg p-3 sm:p-4 bg-purple-50">
                            <div
                                class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 space-y-2 sm:space-y-0">
                                ...
                                <div class="flex space-x-2 text-xs">
                                    <button type="button" id="select-all-specific-years"
                                        class="text-purple-600 hover:text-purple-800 font-medium">
                                        Pilih Semua
                                    </button>
                                    <span class="text-purple-400">|</span>
                                    <button type="button" id="deselect-all-specific-years"
                                        class="text-purple-600 hover:text-purple-800 font-medium">
                                        Hapus Semua
                                    </button>
                                </div>
                            ...
                            <div class="mb-4">
                                <div class="relative">
                                    <input type="text" id="search-specific-years"
                                    ...
                                    <button type="button" id="clear-search-specific-years"
                            ...
                            @if (!empty($graduationYears))
                                <div id="specific-years-container"
                                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 max-h-60 sm:max-h-80 overflow-y-auto">
                                    @foreach ($graduationYears as $year)
                                        <label
                                            class="specific-year-item flex items-center justify-between p-3 bg-white rounded border cursor-pointer hover:bg-purple-50 transition-colors"
                                            data-search-text="{{ $year }}">
                                            <div class="flex items-center flex-1">
                                                <input type="checkbox" name="target_graduation_years[]"
                                                    value="{{ $year }}"
                                                ...
                                                <span
                                                    class="ml-2 text-sm font-medium text-gray-700">{{ $year }}</span>
                                            </div>
                                            <span
                                                class="text-xs text-purple-600 bg-purple-100 px-2 py-1 rounded flex-shrink-0 ml-2">
                    ...
                    <!-- Action Buttons -->
                    <div
                        class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4 sm:pt-6 border-t border-gray-200">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                targetTypeRadios.forEach(radio => {
                    radio.addEventListener('change', function() {
                        ...
                        document.querySelectorAll('.years-ago-checkbox, .specific-year-checkbox')
                            .forEach(cb => {
                                cb.checked = false;
                            });
                ...
                function updatePreview() {
                    ...
                    if (selectedTargetType === 'all') {
                        ...
                        if (selectedYearsAgo.length === 0) {
                            ...
                            alumniCount.textContent =
                                `Total: ${totalSelectedAlumni} alumni dari ${selectedYearsAgo.length} periode`;
                        }
                        ...
                        if (selectedYears.length === 0) {
                            ...
                            alumniCount.textContent =
                                `Total: ${totalSelectedAlumni} alumni dari ${selectedYears.length} tahun kelulusan`;
                        }
                    }
                }
            ...
            document.getElementById('questionnaire-form').addEventListener('submit', function(e) {
                ...
                // Disable tombol agar tidak bisa ditekan lagi
                submitBtn.disabled = true;

                // Optional: ubah teks tombol agar user tahu sedang diproses
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Sedang menyimpan...';
            });
        </script>
