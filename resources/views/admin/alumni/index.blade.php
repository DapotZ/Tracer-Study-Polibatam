    ...
    <x-layout-admin>
        ...
        <div class="px-3 sm:px-4 lg:px-6 max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden p-4 sm:p-6 space-y-6">
                ...
                @if (session('success'))
                    <div
                        class="bg-green-50 border border-green-300 text-green-800 text-sm px-4 py-3 rounded-md flex items-start gap-2 shadow-sm animate-fade-in mt-2">
                ...
                <div id="prodiForm" class="hidden">
                    <form id="addProdiForm" onsubmit="handleAddProdi(event)" class="flex flex-col sm:flex-row gap-3 mt-2">
                        ...
                        <select name="id_department" id="departmentSelect" required
                            class="border border-gray-300 rounded-md px-4 py-2 w-full sm:w-72 text-sm focus:outline-none focus:ring-2 focus:ring-green-300">
                            <option value="">-- Pilih Jurusan --</option>
                            @foreach ($departments as $department)
                                <option value="{{ $department->id_department }}">{{ $department->department_name }}</option>
                            @endforeach
                        </select>
                        <input type="text" name="study_program" required id="newProdiInput"
                ...
                <div id="editProdiForm" class="hidden space-y-2">
                    ...
                    <form action="{{ route('admin.study-program.update') }}" method="POST"
                        ...
                        <select name="id_study" id="editSelect" required
                            ...
                            @foreach ($prodi as $p)
                                <option value="{{ $p->id_study }}" data-name="{{ $p->study_program }}">
                                    {{ $p->study_program }}</option>
                            @endforeach
                        </select>
                ...
                <div id="hapusProdiForm" class="hidden space-y-2">
                    ...
                    <form id="deleteProdiForm" action="{{ route('admin.study-program.deleteBySelect') }}" method="POST"
                        onsubmit="handleDeleteProdi(event)" class="flex flex-col sm:flex-row gap-3">
                        @csrf
                        @method('DELETE')
            ...
            <div class="px-4 sm:px-6 py-4 border-b bg-white mt-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <!-- Tombol Bulk Delete -->
                        <button id="selectAlumniDelete" type="button" onclick="selectAlumniDelete()"
                        ...
                        <button type="button" onclick="deleteAllAlumni()"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="graduation_year"
                                class="border border-gray-300 rounded-md px-3 py-2 w-full text-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="">-- Semua Tahun Lulus --</option>
                                @foreach ($tahunLulus as $tahun)
                                    <option value="{{ $tahun }}"
                                        {{ request('graduation_year') == $tahun ? 'selected' : '' }}>
                                        {{ $tahun }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-1">
                            <label class="text-xs sm:text-sm font-medium text-gray-700 block sm:hidden">Program
                                Studi</label>
                            <select name="id_study"
                                class="border border-gray-300 rounded-md px-3 py-2 w-full text-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="">-- Semua Program Studi --</option>
                                @foreach ($prodi as $p)
                                    <option value="{{ $p->id_study }}"
                                        {{ request('id_study') == $p->id_study ? 'selected' : '' }}>
                                        {{ $p->study_program }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-1">
                            <label
                                class="text-xs sm:text-sm font-medium text-gray-700 block sm:hidden invisible">Action</label>
                            <button type="submit"
                                class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md font-semibold text-sm transition duration-200">
                            ...
                            @forelse($alumni as $index => $item)
                                <tr class="border-t hover:bg-gray-50 transition-colors">
                                    <td class="px-3 py-2 text-center">
                                        <input type="checkbox" name="selected_alumni[]" value="{{ $item->id_user }}"
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        {{ ($alumni->currentPage() - 1) * $alumni->perPage() + $index + 1 }}</td>
                ...
                <div class="hidden md:block lg:hidden overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-100 text-gray-700 uppercase text-xs">
                            <tr>
                                <th class="px-3 py-2 text-center w-12">
                                    <input type="checkbox" id="selectAllCheckboxTablet"
                        ...
                        <tbody class="text-gray-700">
                            @forelse($alumni as $index => $item)
                                <tr class="border-t hover:bg-gray-50 transition-colors">
                                    <td class="px-3 py-2 text-center">
                                        <input type="checkbox" name="selected_alumni_tablet[]"
                                            value="{{ $item->id_user }}" onchange="updateSelectAllStateTablet()"
                                    ...
                                    <td class="px-3 py-2">
                                        ...
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ $item->studyProgram->study_program ?? '-' }}</div>
                                    </td>
                                    <td class="px-3 py-2">
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    <input type="checkbox" name="selected_alumni_mobile[]" value="{{ $item->id_user }}"
            ...
            <div class="p-4 sm:p-6 border-t bg-gray-50">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    ...
                    <div class="order-1 sm:order-2">
                        @if ($alumni->hasPages())
                            <nav class="flex items-center justify-center sm:justify-end space-x-1"
                                aria-label="Pagination">
                                {{-- Previous Page Link --}}
                                @if ($alumni->onFirstPage())
                                    <span
                                        class="px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm text-gray-400 bg-gray-100 rounded cursor-not-allowed">
                                ...
                                @foreach ($alumni->getUrlRange(1, $alumni->lastPage()) as $page => $url)
                                    @if ($page == $alumni->currentPage())
                                        <span
                                            class="px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm font-semibold text-white bg-blue-600 rounded">
                                            {{ $page }}
                                        </span>
                                    @elseif (
                                        $page == 1 ||
                                            $page == $alumni->lastPage() ||
                                            ($page >= $alumni->currentPage() - 2 && $page <= $alumni->currentPage() + 2))
                                        <a href="{{ $url }}"
                                            class="px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50 transition">
                                            {{ $page }}
                                        </a>
        ...
        <script>
            ...
            async function deleteAllAlumni() {
                ...
                if (hasFilters) {
                    confirmHtml =
                        'Apakah Anda yakin ingin menghapus <b>SEMUA</b> alumni yang sesuai dengan filter saat ini?<br><br>';
                    ...
                    if (currentFilters.graduation_year) {
                        confirmHtml +=
                            `<p class="text-sm">• Tahun Lulus: <span class="font-medium">${currentFilters.graduation_year}</span></p>`;
                    }
                    if (currentFilters.id_study) {
                        ...
                        confirmHtml +=
                            `<p class="text-sm">• Program Studi: <span class="font-medium">${selectedOption.text}</span></p>`;
                    }
                    if (currentFilters.search) {
                        confirmHtml +=
                            `<p class="text-sm">• Pencarian: <span class="font-medium">"${currentFilters.search}"</span></p>`;
                    }
                    confirmHtml += '</div>';
                }

                confirmHtml +=
                    '<div class="text-red-600 text-sm font-semibold mt-3">⚠️ Semua data terkait akan ikut terhapus dan TIDAK DAPAT DIKEMBALIKAN!</div>';
            ...
            async function handleEditProdi(event) {
                ...
                try {
                    ...
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        result = {
                            success: response.ok,
                            message: response.ok ? 'Program studi berhasil diperbarui' :
                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        ...
                        if (contentType && contentType.includes('application/json')) {
                            ...
                            result = {
                                success: response.ok,
                                message: response.ok ? `Program studi "${prodiName}" berhasil dihapus` :
            ...
            async function handleAddProdi(event) {
                ...
                try {
                    ...
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        result = {
                            success: response.ok,
                            message: response.ok ? 'Program studi berhasil ditambahkan' :
                                'Gagal menambahkan program studi'
                        };
                    }
                        ...
                        if (result.isConfirmed) {
                            ...
                            try {
                                ...
                                if (contentType && contentType.includes('application/json')) {
                                    ...
                                    result = {
                                        success: response.ok,
                                        message: response.ok ? 'Data berhasil diimport' :
