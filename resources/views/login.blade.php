...
@section('content')
    ...
    <x-translate-widget position="bottom-left" :languages="['en', 'id']" theme="light" />
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-10">
        ...
        <div
            class="flex flex-col-reverse lg:flex-row items-center w-full max-w-6xl mx-auto rounded-lg shadow-lg overflow-hidden bg-white">
            ...
            <div class="w-full lg:w-1/2 bg-theme-secondary p-8 sm:p-10 md:p-12">
                ...
                @php
                    $decryptedPassword = '';
                    try {
                        $decryptedPassword = isset($password) && $password ? decrypt($password) : '';
                    } catch (\Exception $e) {
                        $decryptedPassword = '';
                    }
                @endphp
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="username" name="username" type="text" required autofocus class="input-field"
                                placeholder="Masukkan Username" value="{{ old('username', $username ?? '') }}">
                        </div>
                    </div>

                    <!-- Password -->
                    <div>
                        <label for="password" class="block mb-1 text-sm font-medium text-gray-700">Masukkan Kata
                            Sandi</label>
                        <div class="relative">
