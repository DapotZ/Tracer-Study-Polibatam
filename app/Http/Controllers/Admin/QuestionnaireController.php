...
class QuestionnaireController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $availableYears = Tb_Periode::selectRaw('YEAR(created_at) as year')->distinct()->orderBy('year', 'desc')->pluck('year');
    ...
    public function store(Request $request)
    {
        ...
        // Cek apakah sudah ada periode dengan start_date dan end_date yang sama dalam 5 detik terakhir
        $exists = Tb_Periode::where('start_date', $request->start_date)
            ->where('end_date', $request->end_date)
            ->where('created_at', '>=', $now->subSeconds(5)) // batas 5 detik
            ->exists();

        if ($exists) {
            return back()
                ->withErrors(['duplicate_quick' => 'Anda sudah baru saja membuat periode ini. Mohon tunggu beberapa detik sebelum mencoba lagi.'])
                ->withInput();
        }
        ...
        if ($request->target_type === 'all') {
            ...
            if (!empty($request->target_graduation_years)) {
                $sortedYears = collect($request->target_graduation_years)->sort()->reverse()->values()->toArray();
            ...
            if (!empty($request->years_ago_list)) {
                ...
                $descriptions = collect($sortedYearsAgo)
        ...
        if ($periode) {
            ...
            SendQuestionnaireReminderJob::dispatch($periode->id_periode)->delay($remindAfterStart);
            ...
            if ($remindBeforeEnd->gt(now())) {
                SendQuestionnaireReminderJob::dispatch($periode->id_periode)->delay($remindBeforeEnd);
            }
        }

        return redirect()->route('admin.questionnaire.index')->with('success', 'Periode Questionnaire berhasil dibuat. Pengingat otomatis juga dijadwalkan.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('admin.questionnaire.index')->with('success', 'Periode Questionnaire berhasil diperbarui.');
    }
    ...
    public function destroy($id_periode)
    {
        try {
            ...
            if ($periode->status === 'active') {
                return redirect()->route('admin.questionnaire.index')->with('error', 'Tidak dapat menghapus periode yang sedang aktif.');
            }
            ...
            return redirect()->route('admin.questionnaire.index')->with('success', 'Periode kuesioner berhasil dihapus.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function storeCategory(Request $request, $id_periode)
    {
        ...
        if ($request->boolean('is_status_dependent') && ($request->for_type === 'alumni' || $request->for_type === 'both') && !empty($request->required_alumni_status)) {
        ...
        if ($request->boolean('is_graduation_year_dependent') && ($request->for_type === 'alumni' || $request->for_type === 'both') && !empty($request->required_graduation_years)) {
            $categoryData['required_graduation_years'] = $request->required_graduation_years;
        }

        $category = Tb_Category::create($categoryData);

        return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Kategori berhasil ditambahkan.')->with('scrollToCategory', $category->id_category);
    }
    ...
    public function updateCategory(Request $request, $id_periode, $id_category)
    {
        ...
        return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Kategori berhasil diperbarui.')->with('scrollToCategory', $category->id_category);
    }
    ...
    public function destroyCategory($id_periode, $id_category)
    {
        try {
            ...
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Kategori berhasil dihapus.');
        } catch (\Exception $e) {
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('error', 'Terjadi kesalahan saat menghapus kategori.');
        }
    }
    ...
    public function createQuestion($id_periode, $id_category)
    {
        ...
        return view('admin.questionnaire.question.create', compact('periode', 'category', 'availableQuestions'));
    }
        ...
        try {
            ...
            if ($request->has('has_dependency') && $request->input('has_dependency') == '1') {
                ...
                if ($dependsOn && empty($dependsValue)) {
                    return back()
                        ->withErrors(['depends_value' => 'Pilih minimal satu jawaban yang memicu pertanyaan ini.'])
                        ->withInput();
                }
            }
            ...
            if (($questionType === 'option' || $questionType === 'multiple') && isset($validated['options'])) {
                foreach ($validated['options'] as $index => $optionText) {
                    if (!empty(trim($optionText))) {
                        ...
                        if ($isOtherOption) {
                            $otherBeforeText = isset($validated['other_before_text'][$index]) ? trim($validated['other_before_text'][$index]) : null;
                            $otherAfterText = isset($validated['other_after_text'][$index]) ? trim($validated['other_after_text'][$index]) : null;
                        }
            ...
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Pertanyaan berhasil ditambahkan.')->with('scrollToQuestion', $question->id_question);
        } catch (\Exception $e) {
    ...
    public function editQuestion($id_periode, $id_category, $id_question)
    {
        ...
        return view('admin.questionnaire.question.edit', compact('periode', 'category', 'question', 'availableQuestions'));
    }
    ...
    public function updateQuestion(Request $request, $id_periode, $id_category, $id_question)
    {
        ...
        try {
            ...
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Pertanyaan berhasil diperbarui.')->with('scrollToQuestion', $id_question);
        } catch (\Exception $e) {
            DB::rollBack();
            ...
            return back()
                ->withInput()
                ->with('error', 'Terjadi kesalahan saat memperbarui pertanyaan: ' . $e->getMessage());
        }
    }
    ...
    public function destroyQuestion($id_periode, $id_question)
    {
        try {
            ...
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('success', 'Pertanyaan berhasil dihapus.')->with('scrollToCategory', $categoryId);
        } catch (\Exception $e) {
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('error', 'Terjadi kesalahan saat menghapus pertanyaan.');
        }
    }
    ...
    public function toggleQuestionStatus($id_periode, $id_question)
    {
        try {
            ...
            return redirect()
                ->route('admin.questionnaire.show', $id_periode)
    ...
    public function responses(Request $request, $id_periode)
    {
        ...
        $hasQuestions = Tb_Category::where('id_periode', $id_periode)->whereHas('questions')->exists();

        if (!$hasQuestions) {
            return redirect()->route('admin.questionnaire.show', $id_periode)->with('error', 'Belum ada pertanyaan untuk periode ini.');
        }

        $query = Tb_User_Answers::where('id_periode', $id_periode)->with(['user.alumni.studyProgram', 'user.company']);
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                ...
                $q->orWhere('status', 'like', "%$search%")->orWhereRaw("DATE_FORMAT(created_at, '%d %b %Y') LIKE ?", ["%$search%"]);
                // Search pada relasi alumni
                $q->orWhereHas('user.alumni', function ($qa) use ($search) {
                    $qa->where('name', 'like', "%$search%")->orWhere('nim', 'like', "%$search%");
                });
                // Search pada relasi company
                $q->orWhereHas('user.company', function ($qc) use ($search) {
                    $qc->where('company_name', 'like', "%$search%")->orWhere('nim', 'like', "%$search%");
                });
            });
        }
        $userAnswers = $query->orderBy('created_at', 'desc')->paginate(10)->appends($request->except('page'));
    ...
    public function responseDetail($id_periode, $id_user_answer)
    {
        ...
        $answeredQuestionIds = Tb_User_Answer_Item::where('id_user_answer', $userAnswer->id_user_answer)->pluck('id_question')->unique();
        ...
        foreach ($categories as $category) {
            ...
            $questions = Tb_Questions::where('id_category', $category->id_category)->whereIn('id_question', $answeredQuestionIds)->with('options')->orderBy('order')->get();

            $questionArray = [];

            foreach ($questions as $question) {
                $answerItems = Tb_User_Answer_Item::where('id_user_answer', $userAnswer->id_user_answer)->where('id_question', $question->id_question)->get();
        ...
        return view('admin.questionnaire.response-detail', compact('periode', 'userAnswer', 'userData', 'userType', 'questionsWithAnswers'));
    }
                ...
                if ($firstAnswer->id_questions_options) {
                    ...
                    if (is_numeric($firstAnswer->answer)) {
                        ...
                        if ($ratingOption) {
                            ...
                            $ratingOption = Tb_Question_Options::where('id_question', $question->id_question)->where('option', $firstAnswer->answer)->first();
    ...
    public function remindAllUsers($id_periode)
    {
        ...
        return redirect()
            ->back()
            ->with('success', "Notifikasi pengingat berhasil dikirim ke $sent user (alumni & perusahaan) untuk periode ini.");
    }
    ...
    public function completeDraftAnswers($id_periode)
    {
        try {
            ...
            if ($completedCount > 0) {
                return redirect()
                    ->back()
                    ->with('success', "Berhasil menyelesaikan {$completedCount} jawaban draft yang belum selesai.");
            } else {
