...
use App\Helpers\TracerStudyHelper;
...
class AdminController extends Controller
{
    ...
    ");
        ...
        $respondedQuery = Tb_Alumni::select('id_study', DB::raw('COUNT(DISTINCT tb_alumni.nim) as total'))->join('tb_user_answers', function ($join) {
            $join->on('tb_alumni.id_user', '=', 'tb_user_answers.id_user')->where('tb_user_answers.status', '=', 'completed');
        });
        ...
        $salaryBaseQuery = DB::table('tb_jobhistory')->join('tb_alumni', 'tb_alumni.nim', '=', 'tb_jobhistory.nim')->where('tb_alumni.status', 'bekerja');
        ...
        foreach ($salaries as $salary) {
            if ($salary < 3000000) {
                $salaryRanges['0 - 3jt']++;
            } elseif ($salary < 4500000) {
                $salaryRanges['3jt - 4.5jt']++;
            } elseif ($salary < 5000000) {
                $salaryRanges['4.5jt - 5jt']++;
            } elseif ($salary < 5500000) {
                $salaryRanges['5jt - 5.5jt']++;
            } elseif ($salary < 6500000) {
                $salaryRanges['6jt - 6.5jt']++;
            } elseif ($salary < 7000000) {
                $salaryRanges['6.5jt - 7jt']++;
            } elseif ($salary < 8000000) {
                $salaryRanges['7jt - 8jt']++;
            } elseif ($salary < 9000000) {
                $salaryRanges['8jt - 9jt']++;
            } elseif ($salary < 10000000) {
                $salaryRanges['9jt - 10jt']++;
            } elseif ($salary < 12000000) {
                $salaryRanges['10jt - 12jt']++;
            } elseif ($salary < 15000000) {
                $salaryRanges['12jt - 15jt']++;
            } elseif ($salary < 20000000) {
                $salaryRanges['15jt - 20jt']++;
            } else {
                $salaryRanges['> 20jt']++;
            }
        }
        ...
        $completedAnswersQuery = DB::table('tb_user_answers')->join('tb_alumni', 'tb_user_answers.id_user', '=', 'tb_alumni.id_user')->where('tb_user_answers.status', 'completed');
    ...
    {
        $studyProgram = $request->query('study_program');
        $graduationYear = $request->query('graduation_year_filter');

        // Lakukan filter sesuai dengan data alumni kamu
        $filteredData = Tb_Alumni::query();

        if ($studyProgram) {
            $filteredData->where('id_study', $studyProgram);
        }

        if ($graduationYear) {
            $filteredData->where('graduation_year', $graduationYear);
        }

        $statisticData = [
            'bekerja' => (clone $filteredData)->where('status', 'Bekerja')->count(),
            'tidak_bekerja' => (clone $filteredData)->where('status', 'Tidak Bekerja')->count(),
            'melanjutkan_studi' => (clone $filteredData)->where('status', 'Melanjutkan Studi')->count(),
            'berwiraswasta' => (clone $filteredData)->where('status', 'Berwiraswasta')->count(),
            'sedang_mencari_kerja' => (clone $filteredData)->where('status', 'Sedang Mencari Kerja')->count(),
        ];

        return response()->json($statisticData);
    }
    ...
    public function getTopCompanies(Request $request)
    {
        ...
        $topCompanies = Tb_jobhistory::with('company')
            ->when($studyProgram, function ($query) use ($studyProgram) {
                $query->whereHas('alumni', function ($q) use ($studyProgram) {
                    $q->where('id_study', $studyProgram);
                });
            })
            ->when($graduationYear, function ($query) use ($graduationYear) {
                $query->whereHas('alumni', function ($q) use ($graduationYear) {
                    $q->where('graduation_year', $graduationYear);
                });
            })
            ->select('id_company', DB::raw('COUNT(DISTINCT nim) as total'))
            ->groupBy('id_company')
            ->orderByDesc('total')
            ->take(5)
            ->get()
            ->map(function ($item) {
                return [
                    'company_name' => $item->company->company_name ?? 'Tidak Diketahui',
                    'total' => $item->total,
                ];
            });
    ...
    public function getTopStudyPrograms(Request $request)
    {
        $studyProgram = $request->input('study_program');
        $graduationYear = $request->input('graduation_year_filter');

        $topStudyPrograms = Tb_alumni::where('status', 'bekerja') // hanya alumni yang bekerja
            ->when($studyProgram && $studyProgram !== 'all', function ($query) use ($studyProgram) {
                $query->where('id_study', $studyProgram);
            })
            ->when($graduationYear && $graduationYear !== 'all', function ($query) use ($graduationYear) {
                $query->where('graduation_year', $graduationYear);
            })
            ->select('id_study', DB::raw('COUNT(*) as total'))
            ->groupBy('id_study')
            ->orderByDesc('total')
            ->take(5)
            ->get()
            ->map(function ($item) {
                return [
                    'study_program' => $item->studyProgram->study_program ?? 'Tidak Diketahui',
                    'total' => $item->total,
                ];
            });

        return response()->json($topStudyPrograms);
    }

    public function alumniIndex(Request $request)
    {
        $query = Tb_alumni::with('studyProgram.department');
        $departments = Tb_Department::with('studyPrograms')->get();
        ...
        return view('admin.alumni.index', compact('alumni', 'prodi', 'tahunLulus', 'departments'));
    }
    ...
    public function alumniDestroy($id_user)
    {
        ...
        if (!is_numeric($id_user) || $id_user <= 0) {
            ...
            return redirect()->route('admin.alumni.index')->with('error', 'Parameter ID tidak valid.');
        }
        ...
        if (is_string($id_user) && (strpos($id_user, 'bulk') !== false || $id_user === 'bulk-delete')) {
            ...
            return redirect()->route('admin.alumni.index')->with('error', 'Parameter tidak valid. Silakan refresh halaman dan coba lagi.');
        }
        ...
        if (!$user) {
            ...
            return redirect()->route('admin.alumni.index')->with('error', 'Alumni tidak ditemukan.');
        }
    ...
    public function companyIndex(Request $request)
    {
        ...
        $companies = $query
            ->where(function ($q) {
                $q->whereNotNull('company_address')->orWhereNotNull('company_email')->orWhereNotNull('company_phone_number');
            })
    ...
    public function companyUpdate(Request $request, $id_company)
    {
        $request->validate(
            [
                'company_name' => ['required', 'string', 'max:100', 'unique:tb_company,company_name,' . $id_company . ',id_company'],
    ...
    public function companyExport()
    {
        ...
        // Set headers
        $headers = ['Nama Perusahaan', 'Email', 'Alamat', 'Nomor Telepon', 'Nama HRD'];
        ...
        // Add data
        $row = 2;
        Tb_Company::select('company_name', 'company_email', 'company_address', 'company_phone_number', 'Hrd_name')->chunk(100, function ($companies) use ($sheet, &$row) {
            foreach ($companies as $company) {
                // Kapitalisasi nama HRD saat export
                $hrdName = $company->Hrd_name ? ucwords(strtolower($company->Hrd_name)) : '';
                $sheet->fromArray([$company->company_name, $company->company_email, $company->company_address, $company->company_phone_number, $hrdName], null, 'A' . $row);
                $row++;
            }
        });
    ...
    public function companyTemplate()
    {
        ...
        // Add notes
        $notes = ['Catatan:', '- Email perusahaan wajib diisi dan harus unik', '- Email akan digunakan sebagai username dan password untuk login', '- Format email harus valid (contoh: company@example.com)', '- Format nomor telepon: 021-1234567 atau 0812-3456-7890', '- Nama HRD wajib diisi jika ada, bisa kosong jika tidak diketahui'];
    ...
    public function companyDestroy($id_user)
    {
        ...
        try {
            ...
            return redirect()->route('admin.company.index')->with('success', 'Data perusahaan berhasil dihapus');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    private function getQuestionnaireStatistics(Request $request)
    {
        ...
        $availableYears = Tb_Periode::select(DB::raw('YEAR(start_date) as year'))->distinct()->orderBy('year', 'desc')->pluck('year')->toArray();
        ...
        if ($selectedYear) {
            $periodsInYear = Tb_Periode::whereYear('start_date', $selectedYear)->orWhereYear('end_date', $selectedYear)->orderBy('start_date')->get();
        }
        ...
        if ($selectedYear && $periodsInYear->count() > 0) {
            ...
            foreach ($periodsInYear as $periode) {
                if ($periode->all_alumni || $periode->target_type === 'all') {
                    $yearGraduation = Tb_Alumni::select('graduation_year')->distinct()->whereNotNull('graduation_year')->where('graduation_year', '!=', '')->orderBy('graduation_year', 'desc')->pluck('graduation_year')->toArray();
                    ...
                    $yearGraduation = collect($periode->years_ago_list)
                        ->map(function ($yearsAgo) use ($currentYear) {
    ...
    private function getAllQuestionsFromAllCategoriesInYear($year, $userType, $studyProgramId = null, $graduationYear = null)
    {
        ...
        $periodsInYear = Tb_Periode::whereYear('start_date', $year)->orWhereYear('end_date', $year)->get();
    ...
    public function storeStudyProgram(Request $request)
    {
        $request->validate(
            [
                'study_program' => ['required', 'string', 'max:255', 'unique:tb_study_program,study_program', 'regex:/^[A-Z].*$/'],
                'id_department' => 'required|exists:tb_departments,id_department',
            ],
            [
                'study_program.regex' => 'Nama program studi harus dimulai dengan huruf kapital (contoh: Teknik Mekatronika).',
                'id_department.required' => 'Jurusan wajib dipilih.',
                'id_department.exists' => 'Jurusan yang dipilih tidak valid.',
            ],
        );
    ...
    public function deleteStudyProgramBySelect(Request $request)
    {
        try {
            ...
            return redirect()
                ->back()
                ->with('success', "Program Studi \"{$prodiName}\" berhasil dihapus.");
        } catch (\Illuminate\Validation\ValidationException $e) {
            // Check if this is an AJAX request
            if ($request->expectsJson() || $request->ajax()) {
                return response()->json(
    ...
    public function bulkDeleteAlumni(Request $request)
    {
        ...
        try {
            ...
            $alumniNims = Tb_User::whereIn('tb_user.id_user', $existingUserIds)->where('tb_user.role', 2)->join('tb_alumni', 'tb_user.id_user', '=', 'tb_alumni.id_user')->pluck('tb_alumni.nim')->toArray();
            ...
            $deletedAnswerItems = DB::table('tb_user_answer_item')
                ->whereIn('id_user_answer', function ($query) use ($existingUserIds) {
                    $query->select('id_user_answer')->from('tb_user_answers')->whereIn('id_user', $existingUserIds);
                })
                ->delete();

            $deletedAnswers = DB::table('tb_user_answers')->whereIn('id_user', $existingUserIds)->delete();

            // Delete job history using NIM
            $deletedJobHistory = 0;
            if (!empty($alumniNims)) {
                $deletedJobHistory = DB::table('tb_jobhistory')->whereIn('nim', $alumniNims)->delete();
            }

            // Hapus data dari kedua tabel dengan explicit integer binding
            $deletedAlumni = DB::table('tb_alumni')->whereIn('id_user', $existingUserIds)->delete();
            ...
            return redirect()->route('admin.alumni.index')->with('success', $successMessage);
        } catch (\Exception $e) {
            DB::rollBack();
        ...
        try {
            ...
            $deletedAnswerItems = DB::table('tb_user_answer_item')
                ->whereIn('id_user_answer', function ($query) use ($alumniToDelete) {
                    $query->select('id_user_answer')->from('tb_user_answers')->whereIn('id_user', $alumniToDelete);
                })
                ->delete();

            $deletedAnswers = DB::table('tb_user_answers')->whereIn('id_user', $alumniToDelete)->delete();

            // Delete job history using NIM
            $deletedJobHistory = DB::table('tb_jobhistory')->whereIn('nim', $alumniNims)->delete();

            // Delete alumni data
            $deletedAlumni = DB::table('tb_alumni')->whereIn('id_user', $alumniToDelete)->delete();
    ...
    public function export()
    {
        ...
        // Set headers
        $headers = ['NIM', 'NIK', 'Nama Lengkap', 'Jenis Kelamin', 'Tanggal Lahir', 'Email', 'Nomor Telepon', 'IPK', 'Alamat', 'Angkatan', 'Tahun Lulus', 'Program Studi', 'Status'];
        ...
        Tb_Alumni::with('studyProgram')->chunk(100, function ($alumni) use ($sheet, &$row) {
            foreach ($alumni as $data) {
                $sheet->fromArray([$data->nim, $data->nik, $data->name, $data->gender, $data->date_of_birth, $data->email, $data->phone_number, $data->ipk, $data->address, $data->batch, $data->graduation_year, $data->studyProgram ? $data->studyProgram->study_program : '', $data->status], null, 'A' . $row);
                $row++;
            }
        });
    ...
    public function alumniTemplate()
    {
        ...
        // Add notes
        $notes = ['Catatan:', '- NIM wajib diisi dan harus unik', '- Jenis Kelamin harus diisi dengan: Pria atau Wanita', '- Tanggal Lahir format: YYYY-MM-DD (contoh: 2000-01-01)', '- Status harus diisi dengan salah satu dari: bekerja, tidak bekerja, melanjutkan studi, berwiraswasta, atau sedang mencari kerja', '- Program Studi harus sesuai dengan yang ada di sistem', '- Email harus unik untuk setiap alumni', '- IPK menggunakan format desimal dengan titik (contoh: 3.50)'];
    ...
    {
        $tahunList = Tb_Periode::pluck('start_date')->map(fn($date) => \Carbon\Carbon::parse($date)->format('Y'))->unique()->sortDesc()->values();

        $prodiList = Tb_study_program::pluck('study_program', 'id_study');

        return view('admin.report.index', compact('tahunList', 'prodiList'));
    }

    public function generateReport(Request $request)
    {
        $tahun = $request->input('tahun');
        $selectedProdiIds = $request->input('prodi_ids'); // array of id_study

        if (!$tahun || !$selectedProdiIds) {
            return response()->json(
                [
                    'narasi' => 'Tahun atau Program Studi belum dipilih.',
                    'data' => [],
                ],
                422,
            );
        ...
        // Ambil semua periode berdasarkan tahun
        $periodes = Tb_Periode::whereYear('start_date', $tahun)->get();

        if ($periodes->isEmpty()) {
            return response()->json([
                'narasi' => "Tidak ada data periode untuk tahun $tahun.",
                'data' => [],
            ]);
        }

        // Ambil semua id_periode
        $periodeIds = $periodes->pluck('id_periode');

        // Ambil semua alumni yang eligible dari semua periode di tahun itu
        $eligibleAlumni = collect();
        foreach ($periodes as $periode) {
            $eligibleAlumni = $eligibleAlumni->merge($periode->getEligibleAlumni());
        }
        $eligibleAlumni = $eligibleAlumni->unique('id_user')->values();

        // Filter alumni sesuai prodi terpilih
        $filteredAlumni = $eligibleAlumni->filter(function ($alumni) use ($selectedProdiIds) {
            return in_array($alumni->id_study, $selectedProdiIds);
        });

        // Ambil data prodi
        $prodis = Tb_study_program::whereIn('id_study', $selectedProdiIds)->get();

        // Hitung data per prodi
        $dataProdi = $prodis->map(function ($prodi) use ($filteredAlumni, $periodeIds) {
            $alumniProdi = $filteredAlumni->where('id_study', $prodi->id_study);
            $total = $alumniProdi->count();

            $mengisi = Tb_User_Answers::whereIn('id_user', $alumniProdi->pluck('id_user'))->whereIn('id_periode', $periodeIds)->where('status', 'completed')->distinct('id_user')->count('id_user');

            $tidakMengisi = $total - $mengisi;
            $persen = $total > 0 ? round(($mengisi / $total) * 100, 2) : 0;

            return [
                'prodi' => $prodi->study_program,
                'mengisi' => $mengisi,
        ...
        $narasi = TracerStudyHelper::generateNarasiProdi($tahun, $dataProdi);
    ...
    {
        $tahun = $request->input('tahun');
        $selectedProdiIds = $request->input('prodi_ids');

        if (!$tahun || !$selectedProdiIds) {
            return back()->with('error', 'Tahun dan program studi harus dipilih.');
        }
        ...
        if ($periodes->isEmpty()) {
            return back()->with('error', "Tidak ada data periode untuk tahun $tahun.");
        }
        ...
        $totalTarget = $dataProdi->sum('jumlah');
        $totalResponden = $dataProdi->sum('mengisi');
        $daftarProdi = $dataProdi->pluck('prodi')->implode(', ');

        $narasi = TracerStudyHelper::generateNarasiProdi($tahun, $dataProdi);

        // Buat view PDF
        $view = view('admin.report.report_pdf', [
            'tahun' => $tahun,
            'dataProdi' => $dataProdi,
        ...
        return $pdf->download("Laporan_Tracer_{$tahun}.pdf");
    }

    public function statistikPartisipasi(Request $request)
    {
        $periodes = Tb_Periode::orderByDesc('start_date')->get();
        $prodis = Tb_study_program::all();
        $jurusans = Tb_Department::all();

        $selectedPeriode = $request->input('periode_id');
        $selectedProdi = (array) $request->input('prodi_ids', []);
        $selectedJurusan = (array) $request->input('jurusan_ids', []);
        $mode = $request->input('mode', 'prodi');

        // Pilihan "all"
        if (in_array('all', $selectedProdi)) {
            $selectedProdi = Tb_study_program::pluck('id_study')->toArray();
        }
        if (in_array('all', $selectedJurusan)) {
            $selectedJurusan = Tb_Department::pluck('id_department')->toArray();
        }

        // Gabungkan prodi dari jurusan jika mode prodi
        if ($mode === 'prodi' && !empty($selectedJurusan)) {
            $prodiDariJurusan = Tb_study_program::whereIn('id_department', $selectedJurusan)->pluck('id_study')->toArray();
            $selectedProdi = array_unique(array_merge($selectedProdi, $prodiDariJurusan));
            $request->merge(['prodi_ids' => $selectedProdi]);
        }

        $statistik = [];
        $narasiProfilUmum = null;
        $selectedTahun = now()->year;

        if ($selectedPeriode) {
            $periode = Tb_Periode::find($selectedPeriode);
            if ($periode) {
                $selectedTahun = Carbon::parse($periode->start_date)->year;
            }

            $categories = Tb_category::where('id_periode', $selectedPeriode)->get();
            $includeAlumni = $categories->contains(fn($cat) => in_array($cat->for_type, ['alumni', 'both']));
            $includeCompany = $categories->contains(fn($cat) => in_array($cat->for_type, ['company', 'both']));

            // Alumni
            if ($includeAlumni) {
                $statusDibolehkan = [];
                $tahunLulusDibolehkan = [];

                foreach ($categories as $category) {
                    if (in_array($category->for_type, ['alumni', 'both'])) {
                        if ($category->is_status_dependent && !empty($category->required_alumni_status)) {
                            $statusList = is_array($category->required_alumni_status) ? $category->required_alumni_status : json_decode($category->required_alumni_status, true);
                            $statusDibolehkan = array_merge($statusDibolehkan, (array) $statusList);
                        }
                        if ($category->is_graduation_year_dependent && !empty($category->required_graduation_years)) {
                            $tahunList = is_array($category->required_graduation_years) ? $category->required_graduation_years : json_decode($category->required_graduation_years, true);
                            $tahunLulusDibolehkan = array_merge($tahunLulusDibolehkan, (array) $tahunList);
                        }
                    }
                }

                $tahunLulusFinal = array_unique($tahunLulusDibolehkan);
                $statusDibolehkan = array_unique($statusDibolehkan);

                // Pilih prodi sesuai mode
                if ($mode === 'jurusan') {
                    $prodiIds = Tb_study_program::whereIn('id_department', $selectedJurusan)->pluck('id_study')->toArray();
                } else {
                    $prodiIds = $selectedProdi;
                }

                foreach ($prodiIds as $prodiId) {
                    $prodi = Tb_study_program::find($prodiId);
                    if (!$prodi) {
                        continue;
                    }

                    $queryAlumni = Tb_Alumni::where('id_study', $prodiId);
                    if (!empty($tahunLulusFinal)) {
                        $queryAlumni->whereIn('graduation_year', $tahunLulusFinal);
                    }
                    if (!empty($statusDibolehkan)) {
                        $queryAlumni->whereIn('status', $statusDibolehkan);
                    }

                    $alumni = $queryAlumni->get();
                    $alumniNims = $alumni->pluck('id_user')->toArray();

                    $respondenNims = Tb_User_Answers::where('id_periode', $selectedPeriode)->whereIn('id_user', $alumniNims)->where('status', 'completed')->pluck('id_user')->toArray();

                    $jumlahMengisi = count($respondenNims);
                    $jumlahTotal = count($alumniNims);
                    $jumlahBelum = $jumlahTotal - $jumlahMengisi;
                    $persentase = $jumlahTotal > 0 ? round(($jumlahMengisi / $jumlahTotal) * 100, 2) : 0;

                    $statistik[] = [
                        'prodi' => $prodi->study_program,
                        'jurusan' => $prodi->department->department_name ?? '-',
                        'id_jurusan' => $prodi->id_department,
                        'type' => 'alumni',
                        'mengisi' => $jumlahMengisi,
                        'belum' => $jumlahBelum,
                        'total' => $jumlahTotal,
            ...
            // Company
            if ($includeCompany) {
                $userPerusahaan = Tb_User::where('role', 3)->pluck('id_user')->toArray();
                $companyIds = Tb_Company::whereIn('id_user', $userPerusahaan)->pluck('id_user')->toArray();

                $respondenCompany = Tb_User_Answers::where('id_periode', $selectedPeriode)->whereIn('id_user', $companyIds)->where('status', 'completed')->pluck('id_user')->toArray();

                $jumlahMengisi = count($respondenCompany);
                $jumlahTotal = count($companyIds);
                $jumlahBelum = $jumlahTotal - $jumlahMengisi;
                $persentase = $jumlahTotal > 0 ? round(($jumlahMengisi / $jumlahTotal) * 100, 2) : 0;
            ...
            if ($mode === 'prodi') {
                $request->validate(
                    ...
                    [
                        'prodi_ids.required' => 'Silakan pilih minimal satu prodi.',
                        'prodi_ids.min' => 'Silakan pilih minimal satu prodi.',
                    ],
                );

                $semuaProdiTerpilih = count($selectedProdi) === Tb_study_program::count();
                $narasiProfilUmum = \App\Helpers\TracerStudyHelper::generateNarasiProdi($selectedTahun, $statistik, $semuaProdiTerpilih, $periode);
            } elseif ($mode === 'jurusan') {
    $request->validate([
        'jurusan_ids' => 'required|array|min:1',
    ], [
        'jurusan_ids.required' => 'Silakan pilih minimal satu jurusan.',
        'jurusan_ids.min' => 'Silakan pilih minimal satu jurusan.',
    ]);

    // Group by jurusan (aggregate data dari semua prodi di jurusan)
    $groupedJurusan = collect($statistik)
        ->groupBy('id_jurusan')
        ->map(function ($items) {
            $total = $items->sum('total');
            $mengisi = $items->sum('mengisi');
            $belum = $items->sum('belum');
            $persentase = $total > 0 ? round(($mengisi / $total) * 100, 2) : 0;
    ...
    // **Ini yang penting: override statistik dengan hasil grouping**
    $statistik = $groupedJurusan;

    $semuaJurusanTerpilih = count($groupedJurusan) === count($selectedJurusan);

    $narasiProfilUmum = \App\Helpers\TracerStudyHelper::generateNarasiJurusan(
        $selectedTahun,
        $groupedJurusan,
        $semuaJurusanTerpilih,
        $periode,
        null,
    );
}
        ...
        return view('admin.report.partisipasi', [
            'periodes' => $periodes,
            'prodis' => $prodis,
            'jurusans' => $jurusans,
            'statistik' => $statistik,
            ...
            'narasiProfilUmum' => $narasiProfilUmum,
        ]);
    }
}
